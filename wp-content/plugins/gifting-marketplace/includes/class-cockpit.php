...
class GM_Cockpit {
    ...
    public function __construct() {
        ...
        add_action( 'wp_ajax_gm_cockpit_l1_summary',   [ $this, 'ajax_get_l1_summary' ] );
    ...
    /* Aggregate budget summary for an L1 (across all its departments) */
    public static function get_l1_summary( $user_id, $l1_id ) {
        global $wpdb;

        $allocated = (float) $wpdb->get_var( $wpdb->prepare(
            'SELECT COALESCE(SUM(allocated),0) FROM ' . $wpdb->prefix . self::L2_TABLE
            . ' WHERE l1_id=%d AND user_id=%d',
            $l1_id, $user_id
        ) );

        // Resolve occasion data (used for both budget fallback and item_count)
        $meta_json = $wpdb->get_var( $wpdb->prepare(
            'SELECT meta_json FROM ' . $wpdb->prefix . self::L1_TABLE . ' WHERE id=%d AND user_id=%d',
            $l1_id, $user_id
        ) );
        $l1_meta   = $meta_json ? ( json_decode( $meta_json, true ) ?: [] ) : [];
        $occ_match = null;
        if ( ! empty( $l1_meta['occasion_id'] ) ) {
            $occasions = get_user_meta( $user_id, 'gm_occasions', true ) ?: [];
            foreach ( (array) $occasions as $occ ) {
                if ( isset( $occ['id'] ) && $occ['id'] === $l1_meta['occasion_id'] ) {
                    $occ_match = $occ;
                    break;
                }
            }
        ...
        // Fallback: if no L2 budget is set, use the occasion budget
        if ( $allocated === 0.0 && $occ_match && ! empty( $occ_match['budget'] ) ) {
            $allocated = (float) $occ_match['budget'];
        }

        // Planned item count from the occasion
        $item_count = $occ_match ? (int) ( $occ_match['item_count'] ?? 0 ) : 0;
        ...
        foreach ( $orders as $order ) {
            foreach ( $order->get_items() as $item ) {
                if ( (int) $item->get_meta( 'gm_cockpit_l1_id' ) === (int) $l1_id ) {
                    $spent           += (float) $item->get_total();
                    $items_purchased += (int)   $item->get_quantity();
                }
            }
        }

        // Include cart items tagged to this L1, plus any untagged items (added
        // before an event was selected). Exclude only items explicitly tagged
        // to a different L1. No session lookup needed — avoids race conditions.
        $cart_total    = 0.0;
        $items_in_cart = 0;
        if ( function_exists( 'WC' ) && WC()->cart ) {
            foreach ( WC()->cart->get_cart() as $ci ) {
                $ci_l1 = (int) ( $ci['gm_cockpit_l1_id'] ?? 0 );
                if ( $ci_l1 === (int) $l1_id || $ci_l1 === 0 ) {
                    $cart_total    += (float) $ci['line_total'];
                    $items_in_cart += (int)   $ci['quantity'];
                }
            }
        }
        ...
        return [
            ...
            'remaining'       => max( 0.0, $allocated - $spent - $cart_total ),
            'item_count'      => $item_count,
            'items_purchased' => $items_purchased,
            'items_in_cart'   => $items_in_cart,
            'items_remaining' => $item_count > 0 ? max( 0, $item_count - $items_purchased - $items_in_cart ) : null,
        ];
    }
    ...
    /* ── AJAX: L1-level budget summary (product page cockpit) ───────── */

    public function ajax_get_l1_summary() {
        check_ajax_referer( 'gm_dashboard', 'nonce' );
        $user_id = get_current_user_id();
        if ( ! $user_id ) wp_send_json_error( 'Not logged in' );
        ...
        if ( $owner !== $user_id ) wp_send_json_error( 'Forbidden' );

        wp_send_json_success( self::get_l1_summary( $user_id, $l1_id ) );
    }
    ...
    public function ajax_get_data() {
        ...
        // If no L2 rows, synthesise one from the occasion budget so the cockpit shows the right allocated amount
        if ( empty( $l2_rows ) ) {
            $meta_json = $wpdb->get_var( $wpdb->prepare(
                'SELECT meta_json FROM ' . $wpdb->prefix . self::L1_TABLE . ' WHERE id=%d AND user_id=%d',
                $l1_id, $user_id
            ...
            if ( ! empty( $l1_meta['occasion_id'] ) ) {
                $occasions = get_user_meta( $user_id, 'gm_occasions', true ) ?: [];
                foreach ( (array) $occasions as $occ ) {
                    if ( isset( $occ['id'] ) && $occ['id'] === $l1_meta['occasion_id'] && ! empty( $occ['budget'] ) ) {
                        $synth        = new stdClass();
                        $synth->id        = 0;
                        $synth->name      = 'Total Budget';
                        $synth->allocated = (float) $occ['budget'];
                        $synth->meta_json = '{}';
                        $l2_rows = [ $synth ];
                        break;
                    }
    ...
    public function tag_cart_item( $cart_item_data, $product_id ) {
        ...
        // Fallback: use the session-stored active L1/L2 so items added via
        // WC's ?wc-ajax=add_to_cart (URL-based) also get tagged correctly.
        if ( ! $l1 && function_exists( 'WC' ) && WC()->session ) {
            $l1 = (int) ( WC()->session->get( 'gm_cockpit_l1' ) ?? 0 );
            $l2 = (int) ( WC()->session->get( 'gm_cockpit_l2' ) ?? 0 );
        }
