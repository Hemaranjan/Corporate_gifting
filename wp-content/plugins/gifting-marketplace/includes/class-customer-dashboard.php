...
class GM_Customer_Dashboard {
    ...
    public function maybe_flush_rules() {
        if ( get_option( 'gm_flush_rewrite_rules' ) ) {
            flush_rewrite_rules();
    ...
    public function enqueue() {
        ...
        $on_product    = is_singular( 'product' ) && is_user_logged_in();
        $on_shop       = is_shop() || is_product_category() || is_product_tag();
        $on_cart       = is_cart() || is_checkout();

        if ( ! is_account_page() && ! $is_dokan_dash && ! $on_product && ! $on_shop && ! $on_cart ) return;

        // Google Fonts — needed everywhere we have our UI
        wp_enqueue_style(
            'gm-google-fonts',
        ...
        // Dashboard CSS — account pages, Dokan dash, shop pages, product pages, and cart/checkout (nav uses .gm-cnav styles)
        if ( is_account_page() || $is_dokan_dash || ( $on_shop && is_user_logged_in() ) || $on_product || $on_cart ) {
            wp_enqueue_style(
                'gm-dashboard',
        ...
        // Dashboard JS — account pages and Dokan dashboard only
        if ( is_account_page() || $is_dokan_dash ) {
            wp_enqueue_script(
                'gm-dashboard',
        ...
        // Store-front CSS — browse page, shop/category pages, single product pages, and cart/checkout
        if ( $on_browse || $on_shop || $on_product || $on_cart ) {
            wp_enqueue_style(
                'gm-store-front',
        ...
        // Cockpit assets — browse, shop/category pages, and single product pages
        if ( ( $on_browse || $on_product || ( $on_shop && is_user_logged_in() ) ) && class_exists( 'GM_Cockpit' ) ) {
            ...
            if ( $segment && isset( GM_Cockpit::CONFIG[ $segment ] ) ) {
                ...
                if ( $on_browse ) {
                    $css_deps = [ 'gm-dashboard' ];
                    $js_deps  = [ 'jquery', 'gm-dashboard' ];
                } elseif ( $on_shop ) {
                    $css_deps = [ 'gifting-marketplace', 'gm-google-fonts', 'gm-store-front' ];
                    $js_deps  = [ 'jquery', 'gifting-marketplace' ];
                } else {
                    $css_deps = [ 'gifting-marketplace', 'gm-google-fonts' ];
                    $js_deps  = [ 'jquery', 'gifting-marketplace' ];
                }

                wp_enqueue_style(  'gm-cockpit', GM_URL . 'assets/css/cockpit.css', $css_deps, $ck_v );
                wp_enqueue_script( 'gm-cockpit', GM_URL . 'assets/js/cockpit.js',  $js_deps, $ck_j, true );
