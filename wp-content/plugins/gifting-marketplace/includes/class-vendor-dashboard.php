...
class GM_Vendor_Dashboard {
    ...
    public function __construct() {
        ...
        add_action( 'dokan_new_seller_created',       [ $this, 'flag_new_vendor_for_setup'      ], 10, 2 );
        add_action( 'template_redirect',              [ $this, 'redirect_new_vendor_to_settings']        );
    }
    ...
    public function render_description_fields( $current_user, $profile_info ) {
        ...
        <div class="dokan-form-group">
            <label class="dokan-w3 dokan-control-label">
                <?php esc_html_e( 'Store Industry', 'gifting-marketplace' ); ?>
            </label>
            <div class="dokan-w5 dokan-text-left">
    ...
    public function save_description_fields( $store_id, $dokan_settings ) {
        ...
        /* Clear the new-vendor setup flag once profile has been saved */
        delete_user_meta( $store_id, 'gm_needs_profile_setup' );
    }

    /* ── New vendor onboarding redirect ────────────────────────────── */

    public function flag_new_vendor_for_setup( $user_id, $data ) {
        update_user_meta( (int) $user_id, 'gm_needs_profile_setup', '1' );
    }

    public function redirect_new_vendor_to_settings() {
        if ( ! is_user_logged_in() ) return;
        if ( ! function_exists( 'dokan_is_seller_dashboard' ) ) return;
        if ( ! dokan_is_seller_dashboard() ) return;

        $user_id = get_current_user_id();
        if ( ! dokan_is_user_seller( $user_id ) ) return;
        if ( ! get_user_meta( $user_id, 'gm_needs_profile_setup', true ) ) return;

        /* Already on the store-settings page — clear the flag and let them stay */
        $request_uri = $_SERVER['REQUEST_URI'] ?? '';
        if ( strpos( $request_uri, 'settings/store' ) !== false ) {
            delete_user_meta( $user_id, 'gm_needs_profile_setup' );
            return;
        }

        wp_safe_redirect( dokan_get_navigation_url( 'settings/store' ) );
        exit;
    }
    ...
    public function enqueue_assets() {
        ...
        $is_shop_page     = is_shop() || is_product() || is_product_category() || is_product_tag();
        if ( $is_store_page || $is_store_listing || $is_shop_page ) {
            wp_enqueue_style(
                'gm-store-front',
        ...
        // Store listing page needs cockpit.css (defines .gm-shop-wrap grid) and
        // dashboard.css (resets Astra container padding for custom-nav pages)
        if ( $is_store_listing ) {
            $ck_v = filemtime( GM_PATH . 'assets/css/cockpit.css' ) ?: '1.0.0';
            $db_v = filemtime( GM_PATH . 'assets/css/dashboard.css' ) ?: '1.0.0';
            wp_enqueue_style(
                'gm-cockpit',
                GM_URL . 'assets/css/cockpit.css',
                [ 'gm-google-fonts', 'gm-store-front' ],
                $ck_v
            );
            wp_enqueue_style(
                'gm-dashboard',
