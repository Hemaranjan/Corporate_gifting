...
(function ($) {
    ...
    function initCalendar() {
        ...
        /* ── Detail modal ────────────────────────────────────────── */
        function openDetailModal(occ) {
            var c       = occ.color || '#E8386D';
            var dateObj = new Date(occ.date + 'T00:00:00');
            var mnames  = ['January','February','March','April','May','June',
                           'July','August','September','October','November','December'];
            var dnames  = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
            var dateStr = dnames[dateObj.getDay()] + ', ' + dateObj.getDate() + ' ' +
                          mnames[dateObj.getMonth()] + ' ' + dateObj.getFullYear();
            var diff    = Math.ceil((dateObj - today) / 86400000);
            var dLabel  = diff > 0  ? 'in ' + diff + ' day' + (diff !== 1 ? 's' : '') :
                          diff === 0 ? 'Today!' :
                          Math.abs(diff) + ' day' + (Math.abs(diff) !== 1 ? 's' : '') + ' ago';
            ...
            // Header accent
            $('#gm-det-header').css('border-bottom-color', hexRgba(c, 0.35));
            $('#gm-det-title').html(escH(occ.icon) + '&nbsp;' + escH(occ.title));

            // Date + relative
            $('#gm-det-date').text(dateStr);
            $('#gm-det-rel').text(dLabel)
                .css({ background: hexRgba(c, 0.12), color: c });

            // Budget
            if (occ.budget) {
                $('#gm-det-budget').show()
                    .html('<span>💰 Budget</span><strong>₹' + Number(occ.budget).toLocaleString('en-IN') + '</strong>');
            } else {
                $('#gm-det-budget').hide();
            }

            // Gifts list
            var gifts = (D.giftsBy && D.giftsBy[occ.id]) ? D.giftsBy[occ.id] : [];
            var gh = '';
            if (gifts.length) {
                gifts.forEach(function (g) {
                    gh += '<a href="' + escH(g.product_url || '#') + '" class="gm-det-gift-row" target="_blank">' +
                          '<span class="gm-det-gift-row__icon">🎁</span>' +
                          '<span class="gm-det-gift-row__name">' + escH(g.product_name || 'Gift') + '</span>' +
                          '<svg class="gm-det-gift-row__arrow" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>' +
                          '</a>';
                });
            } else {
                gh = '<p class="gm-det-gifts-empty">No gifts planned yet — browse the shop to add one.</p>';
            }
            $('#gm-det-gifts-list').html(gh);

            // Buttons
            $('#gm-det-remove').data('occ-id', occ.id);
            $('#gm-det-add-gift').attr('href', D.shopUrl || D.browseUrl || '/shop/')
                                  .css('background', c);

            $('#gm-occ-detail-modal').addClass('is-open');
        }

        function closeDetailModal() { $('#gm-occ-detail-modal').removeClass('is-open'); }
        ...
        function saveOccasion() {
            ...
            var itemCount = parseInt($('#gm-occ-items').val())   || 0;

            if (!title) { $('#gm-occ-title').addClass('is-error').trigger('focus'); return; }

            var $btn = $('#gm-occ-save').prop('disabled', true).text('Saving…');

            $.post(D.ajaxUrl, { action: 'gm_save_occasion', nonce: D.nonce, title: title, date: date, icon: icon, color: color, budget: budget, item_count: itemCount })
             .done(function (resp) {
                if (resp.success) {
        ...
        $(document)
            ...
            // Occasion chip → open detail modal
            .on('click', '.gm-chip--o', function (e) {
                if ($(e.target).closest('.gm-chip-del').length) return;
                e.stopImmediatePropagation();
                var id  = $(this).data('id');
                var occ = null;
                for (var i = 0; i < occasions.length; i++) {
                    if (occasions[i].id === id) { occ = occasions[i]; break; }
                }
                if (occ) openDetailModal(occ);
            })
            // Cell click → add modal (skip chips and add-btn)
            .on('click', '.gm-cal-cell', function (e) {
                if ($(e.target).closest('.gm-chip-del, .gm-cal-add, .gm-chip--o').length) return;
            ...
            // Detail modal
            .on('click', '#gm-det-close', closeDetailModal)
            .on('click', '#gm-occ-detail-modal', function (e) { if ($(e.target).is('#gm-occ-detail-modal')) closeDetailModal(); })
            .on('click', '#gm-det-remove', function () {
                var id = $(this).data('occ-id');
                closeDetailModal();
                deleteOccasion(id);
            })
            // Add modal
